...
const LayoutSearchModal: Component<LayoutSearchModalProps> = (props) => {
  ...
  const [selectedLayoutId, setSelectedLayoutId] = createSignal(null);
  ...
  const filteredLayouts = () => {
    ...
    return props.layoutData().filter((layout) => {
      return Object.values(layout).some((value) =>
        value.toString().toLowerCase().includes(lowerQuery)
      );
    });
  ...
  const confirmDelete = () => {
    if (layoutToDelete()) {
      props.onLayoutDelete(layoutToDelete().id);
    ...
    <>
      ...
      >
        <Input
          ...
          value={searchValue()}
          onChange={(value) => {
            setSearchValue(value); // SolidJS fournit directement la valeur ici, pas un événement
          }}
          suffix={
            ...
            {(layout) => (
              <div
                ...
                onClick={() => {
                  ...
                  setSelectedLayoutId(layout.id); // Mettez à jour l'élément sélectionné
                }}
              >
